...
-(void)setupDetailImage
{
    ...
    switch (self.item.accessoryType) {
        case XBSettingAccessoryTypeNone:
            self.detailImageView.x = _cellwidth - self.detailImageView.width - XBDetailViewToIndicatorGap - 2;
            break;
...
- (void)setupDetailText
{
    ...
    switch (self.item.accessoryType) {
        case XBSettingAccessoryTypeNone:
            self.detailLabel.x = _cellwidth - self.detailLabel.width - XBDetailViewToIndicatorGap - 2;
            break;
        case XBSettingAccessoryTypeDisclosureIndicator:
...
{
    if (!_indicator) {
        _indicator = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"rightlink"]];
        _indicator.centerY = self.contentView.centerY;
        _indicator.x = _cellwidth - _indicator.width - XBIndicatorToRightGap;
    }
    return _indicator;
}
...
- (UISwitch *)aswitch
{
    if (!_aswitch) {
        ...
        _aswitch.x = _cellwidth - _aswitch.width - XBIndicatorToRightGap;
