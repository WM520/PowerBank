...
- (void)showGuideViewWithImages:(NSArray *)images
    ...
    if (!show || nil) {
        ...
        [theApp.window addSubview:self.view];
        [theApp.window addSubview:self.pageControl];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ((scrollView.contentOffset.x / kHcdGuideViewBounds.size.width) > 2) {
...
- (void)nextButtonHandler:(id)sender {
    ...
    self.images = nil;
    if (theApp.images) {
        [theApp.images removeAllObjects];
        theApp.images = nil;
    }
}
